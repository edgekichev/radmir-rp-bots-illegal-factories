    ...
    if (initialized && canLoad) {
        ...
        if (CurrentRun.name != "nope" && canLoad) {
            ...
            if (CurrentRun.name == "WaitForAlt") {
                ...
                if (AltCounter > 10) {
                    CurrentRun.name = "backToStart";
                }
            }

            // Бежим обратно в ту точку
            if (CurrentRun.name == "backToStart") {
                if (pathProgress > -1) {
                    float x = paths[stanok - 1][pathProgress][0];
                    float y = paths[stanok - 1][pathProgress][1];
            ...
            if (CurrentRun.name == "sdachaEnd2") {
                RunToPoint(-1601.87377, -607.99896, "sdachaAlt");
                AltCounter = 0;
            }
            ...
            if (CurrentRun.name == "sdachaAlt") {
                ...
                if (AltCounter > 10) {
                    ...
                    if (peresdacha < 3) {
                        CurrentRun.name = "RunOut";
                        peresdacha++;
                    }
                    else {
                        CurrentRun.name = "RunOutGood";
                        peresdacha = 0;
                    }
                }
            }

            if (CurrentRun.name == "RunOut") {
                RunToPoint(-1599.5139, -609.0932, "sdachaEnd2");
            }

            if (CurrentRun.name == "RunOutGood") {
...
bool __stdcall onRecvRpc(stRakNetHookParams* param) {
    if (param->packetId == ScriptRPCEnumeration::RPC_ScrClientMessage) {
        ...
        if (CurrentRun.name != "nope") {
            ...
            if (strdialogText.find("главный склад") != std::string::npos && strdialogText.find("положили изготовленный предмет") != std::string::npos) {
                CurrentRun.name = "RunOutGood";
            }

            if (strdialogText.find("Отнесите готовую продукцию на") != std::string::npos && strdialogText.find("главный склад") != std::string::npos) {
                if (CurrentRun.name == "WaitForAlt") {
                    CurrentRun.name = "Sdacha";
                }
            }
