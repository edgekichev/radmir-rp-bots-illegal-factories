	...
	if (!initialized)
	{
		if (GAME && GAME->GetSystemState() == eSystemState::GS_PLAYING_GAME && SF->getSAMP()->IsInitialized())
		{
			...
			if (canLoad && VSNumber == my_key_table) {
				SF->getSAMP()->getChat()->AddChatMessage(-1, "[{7f6bff}Drug{6b8bff}Bot{FFFFFF}]: Успешно загружен.");
				SF->getSAMP()->getChat()->AddChatMessage(-1, "[{7f6bff}Drug{6b8bff}Bot{FFFFFF}]: Активация: {ff6b6b}/narbot");
				...
				SF->getSAMP()->registerChatCommand("nardebug", [](std::string arg) {
					debugBot = !debugBot;
				});

				SF->getSAMP()->registerChatCommand("narbot", [](std::string arg) {
					if (Bot.Status == NOPE) {
						Bot.Status = LOOKING_FOR_STANOK;
						Bot.Stanok = 0;
						Bot.LastAlt = 0;
						Bot.LastSend = 0;
						SF->getSAMP()->getChat()->AddChatMessage(-1, "[{7f6bff}Drug{6b8bff}Bot{FFFFFF}]: Активирован");
					}
					else {
						Bot.Status = NOPE;
						Bot.Stanok = 0;
						Bot.LastAlt = 0;
						Bot.LastSend = 0;
						SF->getSAMP()->getChat()->AddChatMessage(-1, "[{7f6bff}Drug{6b8bff}Bot{FFFFFF}]: Деактивирован");
					}
					});
			}
			else {
				std::string message = "У тебя нет привязки!\nТвой ключ: " + std::to_string(VSNumber);
				SF->getSAMP()->getChat()->AddChatMessage(-1, "[{7f6bff}Drug{6b8bff}Bot{FFFFFF}]: У тебя нет привязки! Твой ключ: %d", VSNumber);
				MessageBox(
					NULL,
					message.c_str(),
					"Нарко бот!",
		...
		if (Bot.Status != NOPE && canLoad) {
			...
			if (Bot.SendingBuild) {
				if (Bot.LastSend == 0) {
					...
					if ((GetTickCount() - 2000) > Bot.LastSend) {
						if (Bot.BuildSended == 5) {
							Bot.LastSend = 0;
							Bot.SendingBuild = false;
						}
						else {
							Bot.LastSend = GetTickCount();
							if (Bot.BuildSended == 0) {
								SendBuild1();
								SendBuild0();
							}
							if (Bot.BuildSended == 1) {
								SendBuild2();
								SendBuild0();
							}
							if (Bot.BuildSended == 2) {
								SendBuild3();
								SendBuild0();
							}
							if (Bot.BuildSended == 3) {
								SendBuild4();
							}
			...
			case LOOKING_FOR_NEAREST_POINT:
				{
					...
					if (NearestPoint.X != 0 && NearestPoint.Y != 0) {
						SF->getSAMP()->getChat()->AddChatMessage(-1, "[{7f6bff}Drug{6b8bff}Bot{FFFFFF}]: Точка: %d | Длина пути: %d | Станок: %d", Bot.PathProgress, Bot.PathLen, Bot.Stanok);
			...
			case SDACHA_LOOT:
				{
					if (GetTickCount() > Bot.LastAlt + 700) {
						...
						CVector* vect = PEDSELF->GetPosition();
						float dist = CalcDist2d(vect->fX, vect->fY, -1602.806762, -585.13568);

						if (dist > 1.5 && Bot.AltAttempts != 0 && canLoad) {
							RunToCoord(-1602.806762, -585.13568);
						}
						else {
							SendAlt();
							Bot.LastAlt = GetTickCount();
			...
			case PERESDACHA_LOOT:
				{
					if (RunToCoord(-1598.885986, -587.560974)) {
						Bot.Status = SDACHA_LOOT;
						Bot.AltAttempts = 0;
					}
					break;
...
bool __stdcall onRecievePacket(stRakNetHookParams* param)
{
	if (param->packetId == 215 && canLoad)
	{
		...
		if ((0 < length) && (length < 10000) && Bot.Status != NOPE) {
			...
			if (strdialogText.find("Drugs") != std::string::npos && Bot.Status == WAIT_FOR_FREE)  {
...
bool __stdcall onRecvRpc(stRakNetHookParams* param) {
	if (param->packetId == ScriptRPCEnumeration::RPC_ScrClientMessage && canLoad) {
		...
		if (Bot.Status != NOPE) {
			...
			if (strdialogText.find("тнесите продукцию") != std::string::npos && strdialogText.find("главный склад") != std::string::npos && canLoad) {
				Bot.Status = RUN_OT_STANOK;
				Bot.PathProgress = Bot.PathLen - 1;
			}

			if (strdialogText.find("успешно произвели") != std::string::npos && strdialogText.find("аркотики") != std::string::npos && canLoad) {
				Bot.Status = RUN_OT_STANOK;
				Bot.PathProgress = Bot.PathLen - 1;
			}
		}
